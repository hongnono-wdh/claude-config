...
└── site/
    ├── index.html           # 落地页
    └── design-styles.html   # 设计风格百科（ui-ux-pro-max 全 84 种风格图鉴）
```
...
> 实测：装 `claude-config` 时输出会显示 `(+ N dependencies)`，5 个依赖插件（3 能力插件 + TS/Go LSP）自动到位。仅那 12 个独立 skill 因 Claude Code 不支持安装脚本，需单独装（见下方「独立 Skills」或用 AI 提示词让 Claude 代跑）。
...
## 独立 Skills（npx skills add · 12 个）

插件之外，以下 12 个 skill 通过 `npx skills add` 单独安装：

### 设计类
| Skill | 来源 | 用途 |
|-------|------|------|
| frontend-design | anthropics | 前端设计，避免 AI 审美 |
| ui-ux-pro-max | nextlevelbuilder | 99 条 UX 规则 + 色彩/字体系统（[84 种风格图鉴](https://cc.gjs.ink/design-styles)） |
| canvas-design | anthropics | 视觉设计/海报/艺术作品 |
| design-taste-frontend | Leonxlnx/taste-skill | 反模板审美（anti-slop），落地页/作品集/重设计的设计品味 |
...
npx skills add Leonxlnx/taste-skill -a claude-code -g -y --skill design-taste-frontend
